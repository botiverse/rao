...
export function handoffMarkdown(events: readonly SessionEvent[], note: string): string {
  ...
  const sections = [
    ...
    "You are taking over as this project's maintainer in a fresh session. " +
      ...
      "Tool results and private reasoning have been omitted. Do not assume omitted results were successful. " +
  ...
  for (const item of items) {
    switch (item.kind) {
      ...
      case "tool":
        sections.push(
          `### Tool call: ${item.tool}\n\n${item.input ?? "(Arguments not reported.)"}\n\n[Tool result omitted]`,
        );
        break;
      case "turn_end":
  ...
  if (new TextEncoder().encode(markdown).length > MAX_HANDOFF_BYTES) {
    throw new Error(
      "Handoff exceeds the 512 KB limit after removing tool results. No history was truncated; the runtime has not changed.",
    );
  }
  return markdown;
}
